!(function ($) {
  ...
  if (range_slider_max) {
    var sliders = document.querySelectorAll(
      "[data-range-slider='priceSlider']"
    );
    var filterSliders = document.querySelectorAll(
    ...
    for (let i = 0; i < sliders.length; i++) {
      ...
      noUiSlider.create(el, {
        ...
        el.noUiSlider.on("end", function (values, handle) {
          $("[data-range-value='priceSliderValue']").text(
  ...
  $(".pricing-list").each(function (i) {
    ...
    if (list.length > 4) {
      ...
      showLink.on("click", function () {
        ...
        showLink.html(
          showLink.html() === "Show More" ? "Show Less" : "Show More"
        );
      });
    }
  });
          ...
          $.each(data.states, function (key, value) {
            $("." + addedState).append(
              $(`<option data-id="${value.id}"></option>`)
                .val(value.state_content.name)
                .html(value.state_content.name)
            );
          });
            ...
            success: function (data) {
              if (data.cities.length > 0) {
                ...
                $.each(data.cities, function (key, value) {
                  $("." + addedCity).append(
                    $(`<option data-id="${value.id}"></option>`)
                      .val(value.city_content.name)
                      .html(value.city_content.name)
                  );
                });
              }
        ...
        if (data.categories.length > 0) {
          ...
          $.each(data.categories, function (key, value) {
            $(".bringCategory").append(
              $(`<option></option>`)
                .val(value.category_content.slug)
                .html(value.category_content.name)
            );
          });
        }
        $(".request-loader").removeClass("show");
